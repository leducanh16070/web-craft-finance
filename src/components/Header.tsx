    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/50 bg-background/80 backdrop-blur-md">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <button
          ...
          className="md:hidden flex h-10 w-10 items-center justify-center rounded-full bg-secondary/40"
          aria-label="Menu"
        >
        ...
        <div className="md:hidden border-t border-border/50 bg-background">
          <div className="container mx-auto px-4 py-4 flex flex-col gap-2">
            ...
            <div className="flex gap-2 pt-2 border-t border-border/50">
              <Link to="/dang-ky" onClick={() => setOpen(false)} className="flex-1 text-center rounded-full border border-border px-4 py-2 text-sm">Đăng ký</Link>
              <Link to="/dang-nhap" onClick={() => setOpen(false)} className="flex-1 text-center rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground">Đăng nhập</Link>
            </div>
          </div>
        </div>
      )}
